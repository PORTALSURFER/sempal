//! Developer utility to export an embedding training dataset from the local library database.
...
fn run() -> Result<(), String> {
    let Some(mut options) = parse_args(std::env::args().skip(1).collect())? else {
        return Ok(());
    };
    options.split_mode = sempal::dataset::export::SplitMode::Stratified;
    let db_path = options
        .resolved_db_path()
        .map_err(|err| err.to_string())?;
    println!("Using DB: {}", db_path.display());
    let summary = sempal::dataset::export::export_embedding_dataset(&options)
        .map_err(|err| err.to_string())?;
    println!(
...
fn help_text() -> String {
    [
        ...
        "  --val-fraction <f64>    Fraction assigned to val (default: 0.1).",
    ]
    .join("\n")
}
