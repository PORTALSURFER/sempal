...
pub(super) fn handle_edit_fade_handle_drag(
    ...
    if fade_in_mute_active {
        ...
        let delta = (-fade_in_mute_response.drag_delta().x / selection_rect.width()).max(0.0);
        let max_mute = selection.max_fade_in_mute_length();
        let mute_fraction = (start_mute.max(0.0) + delta).min(max_mute);
    ...
    if fade_out_mute_active {
        ...
        let delta = (fade_out_mute_response.drag_delta().x / selection_rect.width()).max(0.0);
