...
pub fn sanitize_wav_bytes(mut bytes: Vec<u8>) -> Vec<u8> {
    ...
    while offset + 8 <= bytes.len() {
        ...
        if id == b"fmt " {
            if let Some(repaired) = repair_fmt_chunk_20_to_18(&mut bytes, offset, chunk_size) {
                return repaired;
            }
            break;
        }
...
fn repair_fmt_chunk_20_to_18(bytes: &mut Vec<u8>, chunk_offset: usize, chunk_size: usize) -> Option<Vec<u8>> {
    if chunk_size != 20 {
        return None;
    }
    let fmt_data = chunk_offset + 8;
    if fmt_data + 20 > bytes.len() {
        return None;
    }
    ...
    let cb_size = u16::from_le_bytes(bytes[fmt_data + 16..fmt_data + 18].try_into().ok()?);
    if cb_size != 0 {
        return None;
    }
    if bytes[fmt_data + 18] != 0 || bytes[fmt_data + 19] != 0 {
        return None;
    }

    // Shrink fmt chunk from 20 -> 18 by removing the final 2 bytes and adjusting the header.
    bytes[chunk_offset + 4..chunk_offset + 8].copy_from_slice(&(18u32).to_le_bytes());
    bytes.drain(fmt_data + 18..fmt_data + 20);
