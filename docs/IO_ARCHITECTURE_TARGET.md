...
| Atomic namespace publication requires a verified no-follow target-root capability, live staging/final co-location, and a qualified descriptor/handle-bound atomic no-replace final claim. | Every staging-producing mode, including a qualified all-atomic path, durably records `CopyValidated` after identity/content validation and before any final-name claim, adoption, publish-ready cleanup, or recovery continuation. Whole-publication atomicity additionally requires every applicable source/input-to-staging transfer and the final claim to use a qualified atomic sequence; any bytewise or otherwise unqualified transfer is `WholePublicationNonAtomic`, including on one volume. Cross-device or otherwise unavailable same-filesystem publication copies completely into destination-local same-folder staging, then uses the same qualified no-replace final claim. |
...
`VisibilityVerified`, the appropriate whole-publication value, and persistent
`PowerLossSynchronizationUnverified` downgrade/evidence. This degraded-visibility publication
may remain `FilesystemPublished` and become eligible for `Terminal` + `Succeeded` after all
required source/global/Harvest/projection/cross-source participants reconcile; it never implies
`PowerLossSynchronized`. Platform benchmarking, fault injection, and the exact set of filesystems
that qualify remain implementation-time validation, not an undecided semantic boundary.
...
| --- | --- | --- |
| `CopyStarted` | `FilesystemStaged` | Copy into destination-local same-folder staging and capture source/staged identities; the staged payload may be incomplete and is not publish-ready. Interruption leaves `PartialNeedsRetry` at this checkpoint. |
| `CopyProgress` | `FilesystemStaged` | Record bounded byte offsets; the staged payload may remain incomplete and is not publish-ready. Interruption leaves `PartialNeedsRetry` at the last durable offset and the next chunk requires a fresh pre-write capacity claim. The final name is not the copy target. |
| `DestinationFlushAttempted` | `FilesystemStaged` | Record the actual destination-local staging flush result and media caveat for the possibly incomplete payload; interruption leaves `PartialNeedsRetry`. |
| `CopyValidated` | `FilesystemStaged` | Verify complete staged identity/length or hash, then durably record this checkpoint for every staging-producing mode, including qualified all-atomic paths. It is the explicit completeness and publish-eligibility gate: only after it may recovery, adoption, publication, or cleanup treat staging as publish-ready. Interruption before it leaves `PartialNeedsRetry` and preserves the payload for retry or audit under the existing recovery rules. |
| `PublishStarted` | `FilesystemStaged` | Only after `CopyValidated`, use the qualified descriptor/handle-bound atomic no-replace final claim. A successful claim records `AtomicNamespace` for the final namespace; a late collision does not modify the existing entry, and interruption leaves `PartialNeedsRetry` with complete staging preserved for live inspection. |
| `PublishObserved` | `FilesystemStaged` | Only after `CopyValidated`, observe the destination namespace through the target-root capability; interruption leaves `PartialNeedsRetry`, and live identity checks determine adoption, collision policy, or audit. The observation does not change the final-claim scope. |
...
All checkpoints through `PublishObserved` therefore remain `FilesystemStaged`; `CopyValidated`
...
3. `FilesystemStaged`: bytes or edit output are in an app-owned destination-local staging
   container on the destination filesystem. The container may be incomplete while copying is
...
| `FilesystemPublished` | None, `RetryPending`, `PartialNeedsRetry`, `AuditRequired`, `CancelRequestedAfterPublish` | Source reconciliation must consume the verified output without repeating filesystem work. A qualified no-replace claim followed by `ReopenedVerified` and `VisibilityVerified` may retain `PowerLossSynchronizationUnverified` and continue as a degraded publication; that downgrade alone does not require `RetryPending` or `PartialNeedsRetry`. Cross-source moves must complete the ordered origin-removal checkpoints. Cancellation remains resumable until required reconciliation or verified compensation completes. |
...
When the qualified final claim has been reopened and content-verified with
`VisibilityVerified`, all required source/global/Harvest/projection/cross-source participants
may reconcile to permit `Terminal` + `Succeeded` while retaining
`PowerLossSynchronizationUnverified`; the downgrade alone does not require
`RetryPending` or `PartialNeedsRetry`. An incomplete or genuinely unverifiable publication or
reconciliation remains nonterminal under the existing retry/partial/audit guards.
...
`FilesystemStaged`; `CopyValidated` is the explicit completeness and publish-eligibility gate,
and `ReopenedVerified` is the participant checkpoint that advances to `FilesystemPublished`.
Cross-source move checkpoints (`DestinationSourceReconciled`,
...
4. Only source=staging=final and a qualified all-atomic source/input-to-staging sequence may
   be considered for `WholePublicationAtomic`. Every staging-producing path must first
   durably record `CopyValidated` after identity/content validation and before any final-name
   claim, adoption, publish-ready cleanup, or recovery continuation. If source != staging, or the
   transfer is bytewise or unqualified (including a same-volume bytewise copy), run
   `NonAtomicCopyValidatePublish`: copy into destination-local same-folder staging, flush and
   validate it through durable `CopyValidated`, then use the qualified no-replace final claim
   before destination reopen/source reconciliation. Record `AtomicNamespace` only if the final claim succeeds,
   ...
   including its durable `CopyValidated` checkpoint before synchronization and the qualified
   final-name claim,
...
| `PowerLossSynchronizationUnverified` | `fsync`/`FlushFileBuffers` unavailable, downgraded, or medium not classifiable | Two branches apply after intent: when the qualified descriptor/handle-bound no-replace final claim succeeds, the final is reopened/content-verified, `VisibilityVerified` is established, and all required source/global/Harvest/projection/cross-source participants reconcile, complete under the existing phase guards as `Terminal` + `Succeeded` while retaining this persistent downgrade/evidence; the downgrade alone does not require `RetryPending` or `PartialNeedsRetry`. When publication or reconciliation is incomplete or genuinely unverifiable, remain nonterminal under the existing `RetryPending`/`PartialNeedsRetry`/`AuditRequired` guards. Never claim `PowerLossSynchronized`. | “File is visible; storage durability could not be verified.” |
...
| Staged-file or namespace synchronization fails | Record the primitive/result. If the qualified descriptor/handle-bound no-replace final claim succeeds and the final is reopened/content-verified with `VisibilityVerified`, continue as a degraded publication under the existing reconciliation guards and retain `PowerLossSynchronizationUnverified`; otherwise retain the incomplete/unverifiable publication under the existing `RetryPending`/`PartialNeedsRetry`/`AuditRequired` guards. | Visibility, atomicity, and power-loss status remain distinct; qualified degraded publication may reach terminal success only after required reconciliation, and incomplete/unverifiable publication may not. |
...
| Remote/removable output is visible but synchronization is unverified | Reopen and verify content. If the qualified descriptor/handle-bound no-replace final claim succeeded, establish `VisibilityVerified`, retain separately recorded `AtomicNamespace` and `WholePublicationNonAtomic`, and after all required source/global/Harvest/projection/cross-source participants reconcile, complete under the existing guards as `Terminal` + `Succeeded` with persistent `PowerLossSynchronizationUnverified`; otherwise remain nonterminal under the existing `RetryPending`/`PartialNeedsRetry`/`AuditRequired` guards. | File is visible; `AtomicNamespace`, `WholePublicationNonAtomic`, and `PowerLossSynchronizationUnverified` remain separate claims/evidence, and no branch claims `PowerLossSynchronized`. |
...
| Status/diagnostics | Stable profile/source ownership codes, `RejectedBeforeIntent` wording that work was not started and is not durable, `NonAtomicNamespace` nonterminal wording, live-identity/transfer-based mode selection, visibility/atomicity/power-loss downgrade wording, both qualified degraded publication that reaches terminal `Succeeded` after required reconciliation with persistent `PowerLossSynchronizationUnverified` and incomplete/unverifiable publication or reconciliation that remains non-success, watcher continuity/audit statuses, cross-source origin/both-copy/identity statuses, staged-preservation cancellation status, WAL watermark/external-reader/reserve/reconstitution statuses, partial wording, retry/reveal/restore/audit actions, restart status continuity, redacted path context, metrics cardinality. |
...
retry/audit status for unqualified publication or incomplete/genuinely unverifiable
reconciliation rather than infer success. A synchronization downgrade alone does not require
retry/audit when the qualified final claim is reopened/content-verified, `VisibilityVerified`
is established, and all required reconciliation completes. No remaining decision may collapse
