...
✅ Correct way to start an eframe app maximized
If you use eframe::run_native (Winit backend)

Use the eframe::NativeOptions field:
...
If you try to maximize inside the app’s setup or first frame → winit may ignore it.

If you use custom window attributes (Winit directly)

Example:

let mut options = eframe::NativeOptions::default();
options.winit_window_builder = Some(
    eframe::winit::window::WindowBuilder::new()
        .with_title("My App")
        .with_maximized(true),
);

eframe::run_native("My App", options, ...)

✔ Use this if you need other advanced window flags
⚠ Not necessary unless you override defaults
⚠ Common mistakes (and why they break)
❌ Maximizing inside setup()

Winit ignores maximize if the window is already shown.

fn setup(&mut self, cc: &eframe::CreationContext<'_>) {
    cc.egui_ctx.send_viewport_cmd(ViewportCommand::Maximize);
}


This does work, but one frame later, so users see a resize flicker.

❌ Setting both initial_window_size and maximized

On Windows and KDE you get:
• double-resize
• wrong aspect ratio
• occasional unmaximized state

Pick one, not both.

👉 If your maximization isn’t working, it’s usually one of these:

You run your own event loop and override winit configs

You set DPI scaling or min window size in a way that conflicts

You maximize too late (during or after rendering first frame)

You set fullscreen + maximized together

If you want, paste your main.rs / eframe::NativeOptions code and I’ll tell you exactly what’s wrong.

🔧 Recommended robust pattern for you

Given how picky winit is, this pattern is the safest:
...
fn main() -> Result<(), eframe::Error> {
    ...
    eframe::run_native(
        "Sample Manager",
        native_options,
        Box::new(|cc| Box::<MyApp>::new(cc)),
    )
}


If you want to force maximization even if winit ignores the initial flag, you can add this fallback:

fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
    if self.first_frame {
        ctx.send_viewport_cmd(egui::ViewportCommand::Maximize);
        self.first_frame = false;
    }
}


This never flickers if your initial window size is None.