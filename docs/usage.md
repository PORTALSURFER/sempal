...
## Hotkeys (focus-aware)
- **Global:** `Space` play/pause; `Esc` stop playback / clear selection; `L` toggle loop; `P` or `Shift + P` move trashed samples to the trash folder; `[` trash selected sample(s); `]` keep selected sample(s); `'` tag selected sample(s) as neutral; `Shift + R` play a random visible sample and auto-play; `Alt + R` toggle sticky random navigation; `Ctrl/Cmd + Shift + R` step backward through random history; `Ctrl/Cmd + /` toggle hotkey overlay; `F11` toggle maximized window; focus chords (press `G` then): `W` waveform, `S` sample browser, `C` collection items, `Shift + S` sources list, `Shift + C` collections list.
