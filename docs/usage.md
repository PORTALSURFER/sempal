...
- Drag the selection handle onto Samples, a folder, or a Collection to export a trimmed clip (saved into the selected folder when applicable).
...
- Collection row menu: Set/Clear export folder, **Sync export**, **Open export folder**, **Rename**, **Delete collection**.
...
- Drag sample rows to collections, back into the browser (for retagging), or onto folders to move files.
...
## Hotkeys (focus-aware)
- **Global:** `Space` play/pause; `Ctrl/Cmd + Space` play from cursor; `Shift + Space` replay from last start; `Esc` stop playback / clear selection; `Ctrl/Cmd + Z` or `U` undo; `Ctrl/Cmd + Y` or `Shift + U` redo; `L` toggle loop; `P` or `Shift + P` move trashed samples to the trash folder; `[` trash selected sample(s); `]` keep selected sample(s); `'` tag selected sample(s) as neutral; `Shift + R` play a random visible sample and auto-play; `Alt + R` toggle sticky random navigation; `Ctrl/Cmd + Shift + R` step backward through random history; `Ctrl/Cmd + Shift + L` copy status log; `Ctrl/Cmd + /` toggle hotkey overlay; `Shift + F1` submit a GitHub issue (connect GitHub first); `F11` toggle maximized window; focus chords (press `G` then): `W` waveform, `S` sample browser, `C` collection items, `Shift + S` sources list, `Shift + C` collections list.
- **Sample browser focus:** `Up/Down` move (or jump randomly when sticky mode is on); `Shift + Up/Down` extend; `Right Arrow` Keep; `Left Arrow` Trash; `Ctrl/Cmd + Right/Left` move across triage columns; `X` toggle selection; `Ctrl/Cmd + A` select all; `F` focus search box; `R` rename focused sample; `N` normalize (overwrite); `D` delete; `C` add focused sample to the selected collection.
