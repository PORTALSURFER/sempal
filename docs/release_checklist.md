...
## Build
- Run `scripts/build_installer.ps1` with access to the latest `yamnet.onnx` and `onnxruntime.dll`.
...
- Confirm `%APPDATA%\.sempal\models\yamnet.onnx` exists after install.
