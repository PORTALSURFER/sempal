...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate golden CLAP embedding data for Rust tests."
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("assets/ml/clap_v1/golden_embedding.json"),
    ...
    try:
        ...
        raise RuntimeError(
            "numpy, torch, and transformers are required (pip install numpy torch transformers)"
        ) from err
    ...
    with torch.no_grad():
        embedding = model.get_audio_features(**inputs)[0].cpu().numpy().astype("float32")
